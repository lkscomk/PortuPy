...
class App:
    def __init__(self, master=None):
        ...
        self.filemenu2.add_command(
            label="Procurar (Ctrl + F)", command=self.abrirOpcaoEncontrar)
        self.tela.bind("<Control-f>", self.abrirOpcaoEncontrar)
        self.filemenu2.add_command(
            label="Ocultar/Mostrar tela Python (Ctrl + D)", command=self.abrirOpcaoTelaPython)
        self.tela.bind("<Control-d>", self.abrirOpcaoTelaPython)
        self.filemenu2.add_command(
            label="Converter Para PortoPy (Ctrl + K)", command=self.codificarParaPortoPy)
        ...
        self.filemenu4.add_command(
            label="Documentação", command=self.abrirDocumentacao)
...
escrever(mensagem)
    ...
    def abrirOpcaoEncontrar(self, event=None):
        ...
        self.encontrar.exibirTela(
            self.editor) if self.opcaoEncontrar else self.encontrar.ocultarTela()
